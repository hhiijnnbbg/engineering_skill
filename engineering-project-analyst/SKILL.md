...
3. **事实锁定**：用户已确认的结论作为项目事实，不重复质疑，见 [knowledge/requirement-rules.md](knowledge/requirement-rules.md)
4. **结构化输出**：所有工程分析输出必须遵循输出规范
5. **面向执行**：最终产出必须能转换为 Codex 可执行的任务
...
| 继续之前分析 | "继续昨天的"、"接着上次"、"上次到哪了" | [workflows/context-continue.md](workflows/context-continue.md) |
...
| 已确认事实、需求结论、防止重复确认 | [knowledge/requirement-rules.md](knowledge/requirement-rules.md) |

change-plan 流程必须加载 engineering-safety.md。analyze 与 change-plan 流程必须加载 requirement-rules.md。

## 第三步：输出规范

所有工程分析输出必须遵循 [templates/analysis-report.md](templates/analysis-report.md) 结构：当前情况 → 需求目标 → 影响范围 → 实现方案 → 风险 → 验证方式 → Codex任务。
...
| [workflows/context-continue.md](workflows/context-continue.md) | 继续模式（长项目续接） |
...
| [knowledge/requirement-rules.md](knowledge/requirement-rules.md) | 已确认事实管理规则 |
